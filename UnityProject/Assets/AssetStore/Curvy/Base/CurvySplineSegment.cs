...
public class CurvySplineSegment : MonoBehaviour
{
    ...
    /// <summary>
    /// Gets whether this Control Point is the first visible Control Point
    /// </summary>
    ...
    /// <summary>
    /// Gets whether this Control Point is the last visible Control Point
    /// </summary>
    ...
    public CurvyConnection ConnectionAny
    {
        get
        {
            ...
            if (ConnectedBy.Count > 0) {
                ...
                {
                    if (!ConnectedBy[i] || ConnectedBy[i].Connection==null || !ConnectedBy[i].Connection.Active || ConnectedBy[i].Connection.Other != this)
    ...
    /// <summary>
    /// Control Points this Control Point is connected to
    /// </summary>
    ...
    {
        if (CurvySpline.Gizmos==CurvySplineGizmos.None) return;
    ...
    /// <summary>
    /// Gets all Control Points that are part of any connection with this CP
    /// </summary>
    ...
    public List<CurvySplineSegment> GetAllConnectionsControlPoints()
    {
        ...
        var cons = GetAllConnections();
        if (cons.Count>0)
            res.Add(this);
        for (int i = 0; i < cons.Count; i++)
        {
            if (!res.Contains(cons[i].Owner))
                res.Add(cons[i].Owner);
            if (!res.Contains(cons[i].Other))
                res.Add(cons[i].Other);
        }
        return res;
    }

    /// <summary>
    /// Gets all Connections this CP is part of
    /// </summary>
    ...
    public List<CurvyConnection> GetAllConnections()
    {
        ...
        {
            res.Add(Connection);
            var conBy = Connection.Other.ConnectedBy;
            for (int i = 0; i < conBy.Count; i++)
                if (!res.Contains(conBy[i].Connection))
                    res.Add(conBy[i].Connection);
        }
        for (int i = 0; i < ConnectedBy.Count; i++)
        ...
        if (cp)
            ...
            if (ccp!=null) {
                ...
                if (type == CurvyConnection.HeadingMode.Auto) {
                    type = (ccp.PreviousControlPoint) ? (ccp.NextControlPoint) ? CurvyConnection.HeadingMode.Sharp : CurvyConnection.HeadingMode.Minus : CurvyConnection.HeadingMode.Plus;
                }
                
                if (type == CurvyConnection.HeadingMode.Minus) {
                    var t = ccp.PreviousTransform;
                    if (t)
                        return t.position;
                }
                else if (type==CurvyConnection.HeadingMode.Plus) {
    ...
    public Vector3 GetNextPosition()
    {
        ...
        if (cp)
            ...
            if (ccp!=null) {
                ...
                var type = con.Heading;
                if (type == CurvyConnection.HeadingMode.Auto) {
                    type = (ccp.NextControlPoint) ? (ccp.PreviousControlPoint) ? CurvyConnection.HeadingMode.Sharp : CurvyConnection.HeadingMode.Plus : CurvyConnection.HeadingMode.Minus;
                }
                
                if (type == CurvyConnection.HeadingMode.Plus) {
                    var t = ccp.NextTransform;
                    if (t)
                        return t.position;
                }
                else if (type==CurvyConnection.HeadingMode.Minus) {
                    var t = ccp.PreviousTransform;
                    if (t)
                        return t.position;
                }
    ...
    /// <summary>
    /// Reset SegmentIndex and Cached Values. INTERNAL, DON'T CALL THIS DIRECTLY!
    /// </summary>
    ...
    /// <summary>
    /// Fill Cache and assign SegmentIndex.INTERNAL, DON'T CALL THIS DIRECTLY!
    /// </summary>
    ...
    /// <summary>
    /// Fills ApproximationDistances. INTERNAL, DON'T CALL THIS DIRECTLY!
    /// </summary>
    /// <returns></returns>
    public float _UpdateLength()
    {
        CurvySplineSegment prev = PreviousControlPoint;
        
        Distance = (prev && prev.IsValidSegment && !IsFirstSegment) ? prev.Distance + prev.Length : 0;
        Length = 0;
        if (IsValidSegment) {
