...
public class CurvySpline : CurvySplineBase
{
    ...
    public CurvyInitialUpDefinition InitialUpVector = CurvyInitialUpDefinition.ControlPoint;
    /// <summary>
    /// Orientation mode
    /// </summary>
    ...
    public CurvySplineSegment LastVisibleControlPoint
    {
        get
        {
            return (Count > 0) ? this[Count-1].NextControlPoint : null;
        }
    }
    ...
    public override Quaternion GetOrientationFast(float tf, bool inverse)
    {
        float localF;
        CurvySplineSegment seg = TFToSegment(tf, out localF);
        return seg.GetOrientationFast(localF,inverse);
    }
    ...
    /// <summary>
    /// Gets the segment a certain TF lies in
    /// </summary>
    /// <param name="tf"></param>
    /// <returns></returns>
    ...
    /// <summary>
    /// Refreshs the spline immediately
    /// </summary>
    /// <remarks>When AutoRefresh is disabled, use this after moving Control Points or changing spline parameters.</remarks>
    ...
    void doRefreshLength()
    {
        ...
        if (Count > 0)
        {
            int l = this[0].ControlPointIndex;
            int h = this[Count - 1].ControlPointIndex;
            
            for (int i = l; i <= h; i++)
                    mLength += this.ControlPoints[i]._UpdateLength();
            
            if (++h < ControlPointCount)
                this.ControlPoints[h]._UpdateLength();
        }
    }
    ...
    /// <summary>
    /// Reload CPS from Hierarchy. INTERNAL, DON'T CALL THIS DIRECTLY!
    /// </summary>
    public void _RefreshHierarchy()
    {
        int cc = Transform.childCount;
        if (ControlPointCount !=cc) {
        ...
        // Reset CPs cache and SegmentIndex
        for (int i = 0; i < mControlPoints.Count; i++) {
    ...
    /// <summary>
    /// Rename CP's based on ControlPoints list index and reset CP's (incl. SegmentIndex). INTERNAL, DON'T CALL THIS DIRECTLY!
    /// </summary>
    public void _RenameControlPointsByIndex()
    {
        for (int i = 0; i < mControlPoints.Count; i++) {
            if (mControlPoints[i] != null) {
                string newName = "CP" + string.Format("{0:000}", i);
                if (mControlPoints[i].name != newName)
                    mControlPoints[i].name = newName;
                mControlPoints[i]._InitializeControlPoint();
            }
        }
    }
